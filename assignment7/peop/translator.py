...
class Translator(object):
  ...
  def __push(self, seg, num):
    ...
    push_operation = self.__push_from("D")

    if seg == "ARGUMENT":
        comment += ["@ARG", "A=M"] + advance + ["D=M"] + push_operation
    elif seg == "CONSTANT":
        comment += [f"@{num}", "D=A"] + push_operation
    elif seg == "LOCAL":
        comment += ["@LCL", "A=M"] + advance + ["D=M"] + push_operation
  ...
  def __pop(self, seg, num):
      ...
      comment = ['#-------  ' + f'{comment:^24}' + '  -------']
      advance = self.__advance_A(num)
      pop_operation = self.__pop_into("D")

      if seg == "ARGUMENT":
          comment += pop_operation + ["@ARG", "A=M"] + advance + ["M=D"]
      elif seg == "LOCAL":
          comment += pop_operation + ["@LCL", "A=M"] + advance + ["M=D"]
  ...
  def __call(self, name, nargs):
      ...
      return_label = f'{name}$ret.{self.__counter}'
      self.__counter += 1
      asm = ['#-------  ' + f'{comment:^24}' + '  -------']
      asm += [f'@{return_label}', 'D=A'] + self.__push_from()
      for seg in ['LCL', 'ARG', 'THIS', 'THAT']:
          asm += [f'@{seg}', 'D=M'] + self.__push_from()
      asm += ['@SP', 'D=M', f'@{nargs}', 'D=D-A', 
              '@5', 'D=D-A', '@ARG', 'M=D','@SP', 'D=M', '@LCL', 'M=D',
              f'@{name}', 'JUMP', f'({return_label})']
      return asm
  ...
  def __return(self):
      ...
      asm = ['#-------  ' + f'{comment:^24}' + '  -------']
      asm += ['@LCL', 'D=M', '@R13', 'M=D',  
                  '@5', 'A=D-A', 'D=M', '@R14', 'M=D', '@SP', 'A=M-1', 'D=M', '@ARG', 'A=M', 'M=D',  
                  '@ARG', 'D=M+1', '@SP', 'M=D', '@R13', 'AM=M-1', 'D=M', '@THAT', 'M=D',  
                  '@R13', 'AM=M-1', 'D=M', '@THIS', 'M=D', '@R13', 'AM=M-1', 'D=M', '@ARG', 'M=D',  
                  '@R13', 'AM=M-1', 'D=M', '@LCL', 'M=D', '@R14', 'A=M', 'JUMP'] 
      return asm
